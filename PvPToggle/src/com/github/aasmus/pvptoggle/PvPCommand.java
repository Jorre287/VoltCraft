...
public class PvPCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if(sender instanceof ConsoleCommandSender) { //check if command sender is console
			...
			if(cmd.getName().equalsIgnoreCase("pvp") && !p.hasPermission("keepinventory.keep") ) {
				if(args.length == 0) {
				...
				p.sendMessage( ChatColor.RED + "" + ChatColor.BOLD +"Voltcraft" + ChatColor.DARK_GRAY + ">> " +ChatColor.GRAY + "You need to " + ChatColor.RED + "disable " + ChatColor.GRAY + "keepinventory to access " + ChatColor.RED + "pvp." );
			}
			return true;
		}
		return false;
